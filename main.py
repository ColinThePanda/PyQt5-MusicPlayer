...
def get_songs_list(music_folder=None):
    ...
    if music_folder is None:
        song_dir = music_folder
    else:
        song_dir = get_music_folder()
    song_dir = os.path.join(song_dir, 'YtSongs')
    if os.path.exists(song_dir):
        for file in os.listdir(song_dir):
            if file.lower().endswith(('.mp3', '.wav')):
                songs.append(os.path.join(song_dir, file))
    return songs
...
class MainWindow(QWidget):
    ...
    def on_song_clicked(self, item):
        ...
        try:
            song_index = songs_lowercase.index((os.path.join(os.path.join((get_music_folder()), 'YtSongs'), song_name).lower() + ".mp3").lower())
        except:
            song_index = songs_lowercase.index((os.path.join(os.path.join((get_music_folder()), 'YtSongs'), song_name).lower() + ".wav").lower())
        try:
